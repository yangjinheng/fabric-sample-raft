...
/**
 * 订购车辆
 * @param {org.acme.vehicle_network.PlaceOrder} placeOrder - 下单交易
 * @transaction
 */
...
/**
 * 更新订单状态
 * @param {org.acme.vehicle_network.UpdateOrderStatus} updateOrderStatus - 更新订单状态的事务
 * @transaction
 */
...
/**
 * 创建要在演示中使用的参与者
 * @param {org.acme.vehicle_network.SetupDemo} setupDemo - 设置演示的事务
 * @transaction
 */
async function setupDemo() { // eslint-disable-line no-unused-vars
    ...
    // 将人员的名的数组转换为具有该名称标识符的 Person 类型的参与者资源的数组
    people = people.map(function (person) {
        return factory.newResource(namespace, 'Person', person);
    });

    // 创建由 vehicles 常量中的 key 标识的 Manufacturer 参与者资源数组
